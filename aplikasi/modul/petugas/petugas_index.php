...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (nama_petugas LIKE '%".$search."%' OR jenis_kelamin LIKE '%".$search."%' OR alamat LIKE '%".$search."%' OR no_telp LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT id, nama_petugas, jenis_kelamin, alamat, no_telp FROM tbl_petugas WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";
	$data_list = $db->select($sql_tampil);

	$sql_total = "SELECT id FROM tbl_petugas WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('tbl_petugas', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
	...
	<div class="col-md-12">
		<div class="box box-primary" id="cetak">
			<div class="box-header with-border" align="center">
				<h3 class="box-title" style=""><b>Data Petugas</b></h3>
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=petugas&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				data-toggle="table"
				data-id-field="id"
				data-url="<?php echo site_url('?m=petugas&ajax=1'); ?>" 
				data-sort-name="id"
				data-sort-order="asc"
				...
				<thead>
					<tr>
						...
						<th data-field="nama_petugas" data-sortable="true" data-valign="middle">Nama</th>
						<th data-field="jenis_kelamin" data-sortable="true" data-valign="middle" data-align="center">Jenis Kelamin</th>
						<th data-field="alamat" data-sortable="true" data-valign="middle">Alamat</th>
						<th data-field="no_telp" data-sortable="true" data-align="center" data-valign="middle">No Telepon</th>

							<!-- <th data-field="aktif" data-sortable="true" data-align="center" data-halign="center" data-valign="middle">Aktif</th>
							<th data-field="avatar" data-formatter="avatar_ft" data-sortable="false" data-align="center" data-halign="center" data-valign="middle">Avatar</th> -->
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=petugas&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
...
function aksi_ft(value, row, index) {
	var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=petugas&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
	if(row.id >= 1) {
