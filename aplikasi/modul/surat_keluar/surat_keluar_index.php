...
if(isset($_GET['ajax'])) {
	...
	if(!empty($search)) {
		$where = " AND (no_agenda LIKE '%".$search."%' OR no_surat LIKE '%".$search."%' OR tgl_kirim LIKE '%".$search."%' OR jenis_surat LIKE '%".$search."%' OR dari LIKE '%".$search."%' OR kepada LIKE '%".$search."%' OR perihal LIKE '%".$search."%' OR tbl_petugas.nama_petugas LIKE '%".$search."%')";
	}
	$sql_limit = " LIMIT ".$offset.",".$limit." ";
	if ( isset($_POST['sort']) && isset($_POST['order']) ) {
		$order_by = " ORDER BY ".$_POST['sort']." ".$_POST['order']." ";
	}
	$sql_tampil = "SELECT tbl_surat_keluar.id, tbl_surat_keluar.no_agenda, tbl_surat_keluar.no_surat, tbl_surat_keluar.tgl_kirim, tbl_surat_keluar.jenis_surat, tbl_surat_keluar.dari, tbl_surat_keluar.kepada, tbl_surat_keluar.perihal, tbl_petugas.nama_petugas
		FROM tbl_surat_keluar
		LEFT JOIN tbl_petugas ON tbl_petugas.id=tbl_surat_keluar.petugas_pengirim  
		WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";

	$data_list = $db->select($sql_tampil);

	$sql_total = "SELECT tbl_surat_keluar.id FROM tbl_surat_keluar 
					LEFT JOIN tbl_petugas ON tbl_petugas.id=tbl_surat_keluar.petugas_pengirim  
					WHERE 1=1 ".$where." ";
...
if(isset($_GET['hapus'])) {
	$id = $_POST['id'];
	$hapus = $db->delete('tbl_surat_keluar', $id);
	if($hapus) {
		echo 'OK';
	}
	exit();
...
set_web('judul', 'Data Surat Keluar');
load_script('table');
load_script('modal');
	...
	<div class="col-md-12">
		<div class="box box-primary" id="cetak">
			<div class="box-header with-border" align="center">
				<h3 class="box-title" style=""><b>Data Surat Keluar</b></h3>
			...
			<div class="box-body">
				...
				<div id="toolbar" class="">
					<a href="<?php echo site_url('?m=surat_keluar&c=tambah'); ?>" class="btn btn-flat btn-success">
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('?m=surat_keluar&ajax=1'); ?>" 
					data-sort-name="id"
					data-sort-order="asc"
					...
					<thead>
						<tr>
							...
							<th data-field="tgl_kirim" data-sortable="true" data-valign="middle">Tanggal kirim</th>
							<th data-field="jenis_surat" data-sortable="true" data-valign="middle">Jenis Surat</th>
							<th data-field="dari" data-sortable="true" data-valign="middle">Dari</th>
							<th data-field="kepada" data-sortable="true" data-valign="middle">Kepada</th>
							<th data-field="perihal" data-sortable="true" data-valign="middle">Perihal</th>
							<th data-field="nama_petugas" data-sortable="true" data-valign="middle">Petugas Pengirim</th>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#link_konfirmasi_hapus').click(function(event) {
			...
			//var id_hapus = 0;
			$.ajax({
				url: '<?php echo site_url('?m=surat_keluar&hapus=1'); ?>',
				type: 'POST',
				dataType: 'html',
				data: {id: id_hapus},
			})
	...
	function aksi_ft(value, row, index) {
		var nsi_out = '<a class="btn btn-flat btn-sm btn-primary" href="<?php echo site_url('?m=surat_keluar&c=ubah&id'); ?>=' + row.id + '" title="Ubah"><i class="fa fa-pencil"></i></a>';
		if(row.id >= 1) {
