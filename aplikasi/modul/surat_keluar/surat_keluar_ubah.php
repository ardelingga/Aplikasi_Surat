...
$query = "SELECT * FROM `tbl_surat_keluar` WHERE `id` = '".$id."' ";
...
if(isset($_POST['submit'])) {
	...
	if(trim($_POST['tgl_kirim']) == '') {
		$form_error[] = 'Tanggal Kirim harus diisi';
	}
	...
	if(trim($_POST['petugas_pengirim']) == '') {
		$form_error[] = 'Petugas Pengirim harus diisi';
	}
	...
	if(empty($form_error)) {
		...
		$data = array(
			...
			'petugas_pengirim' 		=> $_POST['petugas_pengirim']
			);
		if(trim($_POST['upass']) != '') {
		...
		$where = array('id' => $id);
		$update = $db->ubah('tbl_surat_keluar', $data, $where);
		if($update) {
			set_notif('ubah_ok', 1);
			redirect('?m=surat_masuk');
		} else {
	...
	<div class="col-md-12">
		<div class="box box-success">
			<div class="box-header with-border" align="center">
				<h3 class="box-title" style=""><b>Ubah Data Surat Keluar</b></h3>
				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<input type="text" placeholder="Masukan Tanggal Surat" id="tgl_kirim" name="tgl_kirim" class="form-control datepicker" value="<?php echo $row->tgl_kirim; ?>">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<select id="jenis_surat" name="jenis_surat" class="form-control select2" style="width: 200px;" value="<?php echo $row->jenis_surat; ?>">
							...
							<option value="surat_niaga">Surat Niaga</option>
						</select>
					</div>
					...
					<div class="form-group">
						<label for="petugas_pengirim"> Petugas Penerima </label>
						<br>

						<select id="petugas_pengirim" name="petugas_pengirim" class="form-control select2" style="width : 200px;" value="<?php echo $row->petugas_pengirim; ?>">
							<option value=""> Silahkan Pilih</option>
							<?php
							$sql_petugas_pengirim = "SELECT id, nama_petugas FROM tbl_petugas ORDER BY nama_petugas";
							$tbl_petugas = $db->select($sql_petugas_pengirim);
							foreach ($tbl_petugas as $val ) {
								if ($val->id == $row->petugas_pengirim) {
